...
import { createAgent, gemini, createTool, createNetwork, Tool, type Message, createState } from "@inngest/agent-kit";
...
export const codeAgentFunction = inngest.createFunction(
  ...
  async ({ event, step }) => {
    const sandboxId = await step.run("get-sandbox-id", async()=>{
      const sandbox = await Sandbox.create("veltrix-test-2");
    ...
    const messages= await prisma.message.findMany({
      ...
      orderBy: {
        createdAt: "desc", //TODO: Change to "asc" if AI doesn't understand the previous messages
      },
      take: 5,
    });
